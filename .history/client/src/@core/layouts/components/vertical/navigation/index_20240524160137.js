...
const Navigation = props => {
  ...
  const scrollMenu = container => {
    if (container) {
      ...
      if (shadowRef.current) {
        if (scrollPosition > 0 && !shadowRef.current.classList.contains('d-block')) {
          shadowRef.current.classList.add('d-block');
  ...
  const ScrollWrapper = ({ children }) => (
    <PerfectScrollbar
      ...
      containerRef={ref => {
        if (ref) {
          ref.style.overflowY = 'auto'; // Ensure overflowY is set for scrolling
          handleInfiniteScroll(ref);    // Additional manipulations if needed
        }
      }}
      sx={{
        height: '100%',
