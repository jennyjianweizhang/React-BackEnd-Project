...
const List = () => {
  ...
  const ActionsCell = ({ params, anchorEl, handleClose }) => {
    // ----direct to 'edit' page-----//

    const id = params && params.row ? params.row.id : null;
    const renderEditLink = () => {
      if (id) {
          return (
              <Link href={`/invoice/edit/${id}`} passHref>
                  <MenuItem component="a" onClick={handleClose}>Edit</MenuItem>
              </Link>
          );
      }
      return null;
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        ...
        >
          ...
          <Link href={`/invoice/edit/${params.row.id}`} passHref>
            <MenuItem component="a">Edit</MenuItem>
          </Link>
