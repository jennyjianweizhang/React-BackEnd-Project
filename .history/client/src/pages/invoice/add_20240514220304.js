...
const Add = () => {
  ...
  const handleSelectCustomer = (customerId) => {
    const selectedCustomer = customers.find(cust => cust.name === event.target.value);
    if (selectedCustomer) {
      setInvoiceTo(selectedCustomer.name);
      // If you need to do anything else with the selected customer, you can do it here
    }
  };
  const [customers, setCustomers] = useState(["Jordan Stevenson", "Stephanie Burns", "Tony Herrera", "Kevin Patton", "Mrs. Julie Donovan MD"]);
...
// prop identifies which property of the customer state object updated
  ...
  const handleSubmit = (event) => {
    ...
    setCustomers(prevCustomers => [...prevCustomers, customer.name]); 
    setDrawerOpen(false);
    setCustomer({
      name: "",
      company: "",
