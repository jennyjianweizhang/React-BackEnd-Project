...
const Add = () => {
  ...
  const handleChange = (id, field) => (event) => {
    ...
    // updates the field with the new value from the input field
  ...
  const subtotal = 0; // These should be calculated based on items
  const discount = 0; // These should be calculated
  const tax = 0; // These should be calculated
  const total = subtotal - discount + tax; // Example calculation
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={3}>
          <Box sx={{ padding: 2 }}>
            ...
            <Typography variant="body1" sx={{ mt: 1 }}>Tax:</Typography>
