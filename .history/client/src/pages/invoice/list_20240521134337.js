...
const List = () => {
  ...
  const filteredRows = useMemo(() => {
    return invoice.filter((row) => {
      ...
      const isWithinDateRange = (!startDate && !endDate) || (issuedDate >= startDate && issuedDate <= endDate)
  ...
  const ActionsCell = ({ id }) => {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        ...
        <Link href={`/invoice/preview/${id}`} passHref>
          <IconButton
            aria-label="Preview"
            size="small"
            component="a"
        ...
        <Tooltip title="Edit Invoice" sx={{ mt: "0.6rem" }}>
    ...
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <Card elevation={6} sx={{ height: "10rem", overflow: "visible" }}>
          ...
          <CardContent  sx={{
    minHeight: { xs: '300px', sm: '200px', md: '150px' }, // Example heights, adjust as needed
    overflow: 'hidden'
  }}>
            <Grid container spacing={2} >
