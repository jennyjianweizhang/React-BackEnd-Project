...
//     const isWithinDateRange = issuedDate >= startDate && issuedDate <= endDate;
...
function List() {
  // State to hold the IDs of the selected rows
  const [selectedRowIds, setSelectedRowIds] = useState([]);

  // Example row data for the grid
  const rows = [
  ...
  useEffect(() => {
    console.log("Selected rows updated:", selectedRowIds);
  }, [selectedRowIds]);
  ...
  const handleDeleteRows = () => {
    if (selectedRowIds.length > 0) {
      ...
      // Ideally, you would also update the state that holds your rows here or make a backend call
      setSelectedRowIds([]); // Reset selection after deletion
    } else {
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <Typography variant="h4">DataGrid with Selection</Typography>
        <DataGrid
          rows={rows}
          columns={columns}
          pageSize={5}
      ...
      <Grid item xs={12}>
        ...
        <Button variant="contained" color="primary" onClick={handleDeleteRows}>
          Delete Selected (Always Enabled)
        </Button>
      </Grid>
      <Grid item xs={12}>
        <Typography>{selectedRowIds.length} row(s) selected</Typography>
      </Grid>
    </Grid>
  );
}
