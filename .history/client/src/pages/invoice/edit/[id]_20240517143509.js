...
const edit = () => {
  ...
  useEffect(() =&gt; {
    if (id &amp;&amp; permanentCustomers.length &gt; 0) {
      ...
      if (foundInvoice) {
        ...
        setIssueDate(new Date(foundInvoice.issuedDate));
        setDueDate(new Date(foundInvoice.dueDate));
        setInvoiceTo(foundInvoice.client);
  
        // Look for and set customer details
        const customerDetails = allCustomers.find(cust =&gt; cust.client === foundInvoice.client);
        if (customerDetails) {
          setCustomer({
  ...
  }, [id, permanentCustomers, allCustomers]);
  // useEffect(() => {
