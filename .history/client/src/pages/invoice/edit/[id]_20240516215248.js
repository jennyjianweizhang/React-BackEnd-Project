...
//       const foundInvoice = permanentCustomers.find((inv) => inv.id === parseInt(id));
...
const router = useRouter();
  ...
  const permanentCustomers = useSelector((state) => state.invoicesData?.invoice || []);
  const [tempCustomers, setTempCustomers] = useState([]);
  const allCustomers = [...permanentCustomers, ...tempCustomers];
  ...
  // Set the state based on the fetched invoice data
  useEffect(() => {
    if (invoice) {
      setIssueDate(new Date(invoice.issuedDate));
      setDueDate(new Date(invoice.dueDate));
  ...
  const handleSelectCustomer = (event) => {
    const selectedCustomer = allCustomers.find((cust) => cust.client === event.target.value);
    if (selectedCustomer) {
      setInvoiceTo(selectedCustomer.client);
    }
  };
  ...
  const isValidPhone = (phone) => /^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$/.test(phone);
  ...
  const handleSubmit = (event) => {
    event.preventDefault();
    const formIsValid = Object.values(errors).every((x) => x === '') && Object.values(customer).every((x) => x !== '');
    if (!formIsValid) {
      console.log('Form is invalid: ', errors);
      return;
    }

    const newCustomer = { ...customer, client: customer.name };
