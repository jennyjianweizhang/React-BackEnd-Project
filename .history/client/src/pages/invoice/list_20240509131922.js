  ...
  const ClientCell = (params) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 10}}>
        <Avatar sx={{ bgcolor: 'rgba(105, 108, 255, 0.16)', mt:'-1rem', color:'rgb(105, 108, 255)'}}>{initials}</Avatar>
        <div style={{marginTop:'-0.5rem'}}>
          <div >{params.value}</div>
          <div style={{ fontSize: 'small', color: 'gray', marginTop: '-2rem'}}>{params.value}@example.com</div>
        </div>
      </div>
    );
  };
