...
const List = () => {
  ...
  useEffect(() => {
    ...
    setInitialRows(generatedRows);
    setRows(generateFilteredRows()); 
  }, []);


  useEffect(() => {
    // Filter rows whenever selectedStatus changes
    const filteredRows = selectedStatus ? initialRows.filter(row => row.status === selectedStatus) : initialRows;
    setRows(filteredRows);
  }, [selectedStatus, initialRows]);
