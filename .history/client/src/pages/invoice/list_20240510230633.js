...
const List = () => {
  ...
  const rows = [{ id: 1, name: 'Hello' }, { id: 2, name: 'World' }];
  const columns = [{ field: 'id', headerName: 'ID', width: 90 }, { field: 'name', headerName: 'Name', width: 150 }];

  
  const handleRowClick = (params) => {
    console.log("Row clicked:", params.id);
    const newSelection = selectedRowIds.includes(params.id)
      ? selectedRowIds.filter(id => id !== params.id)
      : [...selectedRowIds, params.id];
