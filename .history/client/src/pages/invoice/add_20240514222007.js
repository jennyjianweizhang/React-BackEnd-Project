...
const Add = () => {
  ...
  const handleSelectCustomer = (event) => {
    const selectedCustomer = allCustomers.find(cust => cust.name === event.target.value);
    if (selectedCustomer) {
...
// prop identifies which property of the customer state object updated
  ...
  const handleSubmit = (event) => {
    ...
    setTempCustomers(prev => {
      const updatedCustomers = [...prev, { ...customer, name: customer.name }];
      console.log("Updated Temporary Customers: ", updatedCustomers);
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      >
                        ...
                        {allCustomers.map((cust, index) => (
                          <MenuItem key={index} value={cust.name}>
                            {cust.name}
                          </MenuItem>
                        ))}
                      </Select>
