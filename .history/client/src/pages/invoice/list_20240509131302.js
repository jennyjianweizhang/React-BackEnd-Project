...
const List = () => {
  ...
  const ClientCell = (params) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 10}}>
        <Avatar sx={{ bgcolor: 'rgba(105, 108, 255, 0.16)', mt:'-3rem', color:'rgb(105, 108, 255)'}}>{initials}</Avatar>
        <div>
          <div style={{marginBottom:'-1rem'}}>{params.value}</div>
          <div style={{ fontSize: 'small', color: 'gray' }}>email@example.com</div>
        </div>
      </div>
    );
  };
  ...
  const columns = [
    ...
    {field: "status", headerName: <StatusIcon />, width: 100,},
    { field: "client", headerName: "CLIENT", width: 200, renderCell: ClientCell },
  ...
  const getRandomClient = () => {
    const names = ["John Doe", "Jane Smith", "Alice Johnson", "Bob Brown", "Carol White", "Dave Black", "Eve Green", "Frank Wright", "Grace Hall", "Hank Moody"];
