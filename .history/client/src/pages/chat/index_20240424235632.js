...
import { fetchAllData } from "src/store/chatData";
import contactInfo from "src/@core/data/datasetContact";
import { addMessage } from "src/store/chatData";
...
const ChatRoom = () => {
  ...
  const { allData } = useSelector((state) => state.chatData);
  const isLoading = useSelector((state) => state.chatData.isLoading);
  ...
  console.log(currentChatData, currentChatData?.messages);
