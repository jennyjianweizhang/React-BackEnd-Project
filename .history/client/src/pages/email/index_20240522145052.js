...
const Email = () => {
  ...
  const unreadInboxCount = allData.filter(
    (email) => email.status.includes("inbox") && !email.read
  ).length;

  const unreadDraftCount = allData.filter(
    (email) => email.status.includes("draft") && !email.read
  ).length;

  const unreadSpamCount = allData.filter(
    (email) => email.status.includes("spam") && !email.read
  ).length;
    ...
    >
      ...
      {isMobile && (
        <IconButton
          ...
          sx={{ marginLeft: 1, marginTop: 2, position: 'absolute', zIndex: theme.zIndex.drawer}}
        >
          <MenuIcon />
        </IconButton>
      )}
        ...
        {isMobile && (
          <IconButton
            ...
            sx={{
              ...
              top: 8,
              color: 'grey.900',
              backgroundColor: 'rgb(105, 108, 255)', // Custom background color
              '&:hover': {
                backgroundColor: 'rgba(0,0,0,0.08)', // Darker on hover
              }
            }}
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" sx={{ bgcolor:'rgb(105, 108, 255)'  }}>
              <path fill="currentColor" d="M13.293 6.293L7.586 12l5.707 5.707l1.414-1.414L10.414 12l4.293-4.293z"></path>
            </svg>
          </IconButton>
        )}
        <Box sx={{ p: 2 }}>
