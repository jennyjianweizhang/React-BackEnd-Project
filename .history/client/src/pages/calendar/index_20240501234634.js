...
const AppCalendar = () => {
  const [eventsToUpdate, setEventsToUpdate] = useState(events);
  ...
  const handleEventClick = (clickInfo) => {
    ...
    setTitle(title);
    setCategory(extendedProps.category);
    setStartDate(new Date(start));
    setEndDate(new Date(end));
    setAllDay(extendedProps.allDay || false);
  ...
  const handleUpdateEvent = () => {
    const updatedEvents = eventsToUpdate.map(event => {
      if (event.id === selectedEvent.id) {
        return { ...event, title, category, start: startDate, end: endDate, allDay, description };
      }
      return event;
    });

    setEventsToUpdate(updatedEvents);
    closeDrawer();
  };
              ...
              >
                ...
                <Box>
                  ...
                  <TextField
                    ...
                    type="date"
                    label="Start Date"
                    value={selectedEvent.start.split("T")[0]} // Assuming ISO string format
                    onChange={(e) =>
                      setSelectedEvent({
                        ...selectedEvent,
                  ...
                  <TextField
                    ...
                    type="date"
                    label="End Date"
                    value={selectedEvent.end.split("T")[0]} // Assuming ISO string format
                    onChange={(e) =>
                      setSelectedEvent({
                        ...selectedEvent,
                        end: e.target.value,
