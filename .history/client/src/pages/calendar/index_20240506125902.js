...
const AppCalendar = () => {
  ...
  const handleEventClick = (clickInfo) => {
    ...
    const formatDateForInput = (dateStr) => {
        // Assuming dateStr is in ISO format 'YYYY-MM-DDTHH:mm:ssZ'
        return dateStr.slice(0, 16);  // Extracts 'YYYY-MM-DDTHH:mm'
      };
    setSelectedEvent({
      id: event.id,
      title: event.title,
      start: formatDateForInput(event.startStr),
      end: formatDateForInput(event.endStr || event.startStr),
          ...
          >
            ...
            {selectedEvent && (
              ...
              >
                ...
                >
                  <Typography variant="body1" sx={{ml:'-0.5rem'}}>{selectedEvent.title}</Typography>
                  <Box sx={{mr:'-1rem'}}>
