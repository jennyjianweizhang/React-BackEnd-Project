...
const AppCalendar = () => {
  ...
  const formatDateForInput = (date) => {
    ...
    try {
        // Check if date is a string; if so, parse it, otherwise use it directly
        const dateObj = typeof date === 'string' ? parseISO(date) : date;
        return format(dateObj, 'yyyy-MM-dd\'T\'HH:mm'); // Format for datetime-local input
    } catch (error) {
  ...
  const handleChange = (field, value) => {
    ...
    if (field === 'start') {
      setStart(formatDateForInput(value));
  } else if (field === 'end') {
      setEnd(formatDateForInput(value));
  }
  };
