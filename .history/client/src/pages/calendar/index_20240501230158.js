...
const AppCalendar = () => {
  ...
  const handleEventClick = (clickInfo) => {
    console.log('Event Clicked:', clickInfo.event); // Debug: log the event data
    const { id, title, start, end, extendedProps: { guests, category } } = clickInfo.event;
  
    // Ensure start and end are valid dates by parsing them first
    ...
    if (!parsedStart || !parsedEnd || isNaN(parsedStart.getTime()) || isNaN(parsedEnd.getTime())) {
      console.error('Invalid date found in event:', { start, end });
