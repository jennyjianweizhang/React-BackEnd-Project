...
const AppCalendar = () => {
  ...
  const handleChangeDateTime = (field, value) => {
    const newDateISO = formatISO(new Date(value), {
      representation: "complete",
    });
    if (
      field === "start" &&
      isBefore(parseISO(selectedEvent.end), parseISO(newDateISO))
    ) {
      handleChange("end", newDateISO);
    }
    handleChange(field, newDateISO);
  };
              ...
              >
                ...
                <Box>
                  ...
                  <TextField
                    ...
                    type="date"
                    label="Start Date"
                    value={selectedEvent.start ? formatISO(parseISO(selectedEvent.start), { representation: 'complete' }).slice(0, 16) : ''}
                    onChange={(e) =>
                      handleChangeDateTime("start", e.target.value)
                    }
                    margin="normal"
                  ...
                  <TextField
                    ...
                    type="date"
                    label="End Date"
                    value={selectedEvent.end ? formatISO(parseISO(selectedEvent.end), { representation: 'complete' }).slice(0, 16) : ''}
    onChange={(e) => handleChangeDateTime('end', e.target.value)}
    min={selectedEvent.start ? formatISO(parseISO(selectedEvent.start), { representation: 'complete' }).slice(0, 16) : ''}
                    margin="normal"
                    InputLabelProps={{ shrink: true }}
                  />
