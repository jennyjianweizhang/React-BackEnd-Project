  ...
  const handleCategoryChange = (category) => {
    setSelectedCategories(prev => {
      if (prev.includes(category)) {
        return prev.filter(c => c !== category); // Remove category if already included
      } else {
        return [...prev, category]; // Add category if not already included
      }
    });
  };
  ...
  return (
    <Box className="app-calendar" sx={{ display: "flex" }}>
      ...
      >
        ...
        <Box sx={{ padding: 2, ml: 5 }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "column" }}>
            <FormControlLabel
              control={<Checkbox checked={selectedCategories.includes('Personal')}
      onChange={() => handleCategoryChange('Personal')} color="primary" />}
              label="View All"
            />
            <FormControlLabel
              control={<Checkbox checked={selectedCategories.includes('Personal')}
      onChange={() => handleCategoryChange('Personal')} color="error" />}
              label="Personal"
            />
            <FormControlLabel
              control={<Checkbox checked={selectedCategories.includes('Personal')}
      onChange={() => handleCategoryChange('Personal')} color="primary" />}
            ...
            <FormControlLabel
              control={<Checkbox checked={selectedCategories.includes('Personal')}
      onChange={() => handleCategoryChange('Personal')} color="warning" />}
              label="Family"
            />
            <FormControlLabel
              control={<Checkbox checked={selectedCategories.includes('Personal')}
      onChange={() => handleCategoryChange('Personal')} color="success" />}
              label="Holiday"
            />
            <FormControlLabel
              control={<Checkbox checked={selectedCategories.includes('Personal')}
      onChange={() => handleCategoryChange('Personal')} color="info" />}
              label="ETC"
            />
          </Box>
        </Box>
