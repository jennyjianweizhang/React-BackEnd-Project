...
const AppCalendar = () => {
  ...
  const [calendarType, setCalendarType] = useState(events.extendedProps.catgory);
  const [allDay, setAllDay] = useState(true);
  const [start, setStart] = useState(events.startDate);
  const [endDate, setEndDate] = useState(events.endDate);
  const [guests, setGuests] = useState(events.extendedProps.guests);
