...
const handleTitleChange = (e) => {
  ...
  const handleEventClick = (clickInfo) => {
    ...
    console.log("Clicked event start:", event.startStr);  // Formatted string of the start
    console.log("Clicked event end:", event.endStr);  // Formatted string of the end

    setSelectedEvent({
        id: event.id,
        title: event.title,
        start: event.startStr,
        end: event.endStr || event.startStr, // Fallback to start if end is null
...
const handleChange = (field, value) => {
    if (field === 'start' || field === 'end') {
        // Adding safety check for valid date inputs
        if (!isNaN(new Date(value).getTime())) {
            const date = new Date(value);
            value = new Date(date.getTime() - (date.getTimezoneOffset() * 60000)).toISOString();
        } else {
            value = null;  // Explicitly handle invalid dates
        }
    }
