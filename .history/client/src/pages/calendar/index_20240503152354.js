...
const AppCalendar = () => {
  ...
  const handleEventClick = (clickInfo) => {
    ...
    setSelectedEvent({
      id,
      title,
      start: format(parseISO(start), "yyyy-MM-dd'T'HH:mm"),
      end: format(parseISO(end), "yyyy-MM-dd'T'HH:mm"),
      ...extendedProps,
    });
  ...
  const formatDateForInput = (dateString) => {
    try {
      // Check if dateString is already a valid ISO string (basic check could be improved)
      if (dateString && dateString.includes('T')) {
        // It's likely an ISO string, parse it to ensure it's a Date and reformat to drop seconds
        return formatISO(parseISO(dateString), { representation: 'complete' }).slice(0, 16);
      }
      // Fallback if dateString is not set or invalid
      return '';
    } catch (error) {
