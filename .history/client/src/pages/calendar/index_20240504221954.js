...
const AppCalendar = () => {
  ...
  const events = useSelector(state => state.eventsData?.events || [], (left, right) => {
    return JSON.stringify(left) === JSON.stringify(right); // Custom comparison
  });
  ...
  useEffect(() => {
    if (selectedEvent) {
        console.log("Selected Event Guests (should be array):", selectedEvent.guests);
      setTitle(selectedEvent.title);
      setCategory(selectedEvent.category);
      setGuests(
        selectedEvent.guests
          ? selectedEvent.guests.split(",").map((guest) => guest.trim())
          : []
      );
