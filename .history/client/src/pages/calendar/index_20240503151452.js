...
const AppCalendar = () => {
  ...
  const formatDateForInput = (date) => {
    if (!(date instanceof Date)) return ''; // Return an empty string if not a Date object
  
    const offset = date.getTimezoneOffset();
    const adjustedDate = new Date(date.getTime() - offset * 60 * 1000);
    return adjustedDate.toISOString().slice(0, 16); // Cuts off seconds and timezone information
  };
              ...
              >
                ...
                <Box>
                  ...
                  <TextField
                    ...
                    value={
                      selectedEvent.end
                        ? formatISO(parseISO(selectedEvent.end), {
                            representation: "complete",
                          }).slice(0, 16)
                        : ""
                    }
                    onChange={(e) =>
                      handleChangeDateTime("end", e.target.value)
                    }
                    min={
                      selectedEvent.start
                        ? formatISO(parseISO(selectedEvent.start), {
