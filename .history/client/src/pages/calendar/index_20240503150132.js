...
const AppCalendar = () => {
  ...
  const handleChangeDate = (field, value) => {
    const newDateISO = format(new Date(value), 'yyyy-MM-dd'); // Format to ISO-like string for consistency

    // If changing start date, make sure end date is adjusted if it is before the new start date
    if (field === 'start' && isBefore(parseISO(selectedEvent.end), parseISO(newDateISO))) {
...
return (
    <TextField
        ...
        type="date"
        label="Start Date"
        value={selectedEvent.start ? format(parseISO(selectedEvent.start), 'yyyy-MM-dd') : ''}
    ...
    <TextField
        ...
        type="date"
        label="End Date"
        value={selectedEvent.end ? format(parseISO(selectedEvent.end), 'yyyy-MM-dd') : ''}
        onChange={(e) => handleChangeDate('end', e.target.value)}
        min={selectedEvent.start ? format(parseISO(selectedEvent.start), 'yyyy-MM-dd') : ''}
        margin="normal"
        InputLabelProps={{ shrink: true }}
    />
);
              ...
              >
                ...
                <Box>
                  ...
                  <TextField
                    ...
                    value={
                      selectedEvent.start
                        ? format(parseISO(selectedEvent.start), "yyyy-MM-dd")
                        : ""
                    }
                    onChange={(e) => handleChangeDate("start", e.target.value)}
                    margin="normal"
