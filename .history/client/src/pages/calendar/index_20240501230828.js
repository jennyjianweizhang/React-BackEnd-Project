...
const AppCalendar = () => {
  ...
  const handleEventClick = (clickInfo) => {
    ...
    // Safely parse dates using parseISO only if they exist
    const start = event.start ? parseISO(event.start.toISOString()) : undefined;
    const end = event.end ? parseISO(event.end.toISOString()) : undefined;
  
    // Access extendedProps and ensure it's not undefined
    const { guests, category } = event.extendedProps || {};
  
    // Prepare ISO strings if dates are valid
    const startISO = start ? start.toISOString() : undefined;
