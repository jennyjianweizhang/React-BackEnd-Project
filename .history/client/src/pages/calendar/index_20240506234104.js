...
import { updateEvent, deleteEvent, addEvent } from "src/store/eventsData";
      ...
      dispatch(
        updateEvent({
          ...
          changes: {
            ...
            description: description,
            classNames: [`event-${category.toLowerCase()}`],
            guests: guests,
          },
        })
      );
  ...
  const handleAddEventClick = () => {
    ...
    const filteredEvents = useMemo(() => {
        ...
        return events.filter(event =>
            (filters.personal && event.extendedProps.category?.toLowerCase() === 'Personal') ||
            (filters.business && event.extendedProps.category?.toLowerCase() === 'Business') ||
            (filters.family && event.extendedProps.category === 'Family') ||
            (filters.holiday && event.extendedProps.category === 'Holiday') ||
            (filters.etc && event.extendedProps.category === 'ETC')
        );
    }, [events, filters]);
    ...
    const filteredEvents = useMemo(() => {
        ...
        return events.filter(event =>
            (filters.personal && event.category?.toLowerCase() === 'personal') ||
            (filters.business && event.category?.toLowerCase() === 'business') ||
            (filters.family && event.category?.toLowerCase() === 'family') ||
            (filters.holiday && event.category?.toLowerCase() === 'holiday') ||
            (filters.etc && event.category?.toLowerCase() === 'etc')
        );
    }, [events, filters]);
    ...
    const handleCheckboxChange = (event) => {
        ...
        if (name === "viewAll") {
            // If toggling 'View All', set all categories to the same value
            setFilters({
                viewAll: checked,
                personal: checked,
                business: checked,
            ...
            // Check if all other categories are true, set 'viewAll' accordingly
            const allChecked = Object.values(updatedFilters).slice(1).every(Boolean); // Skip the first 'viewAll' property
            setFilters({
                ...updatedFilters,
                viewAll: allChecked,
            });
      ...
      console.log(events.map(event => event.category)); 
        ...
        <Box sx={{ padding: 2, ml: 5 }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "column" }}>
            <FormControlLabel
              control={
                <Checkbox
                checked={filters.viewAll} onChange={handleCheckboxChange}
                  color="primary"
                  name="viewAll" 
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox
                checked={filters.personal} onChange={handleCheckboxChange}
                  color="error"
                  name="personal"
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox
                checked={filters.business} onChange={handleCheckboxChange}
                  color="primary"
                  name="business"
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox
                checked={filters.family} onChange={handleCheckboxChange}
                  color="warning"
                  name="family"
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox
                checked={filters.holiday} onChange={handleCheckboxChange}
                  color="success"
                  name="holiday"
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox
                checked={filters.etc} onChange={handleCheckboxChange}
                  color="info"
                  name="etc"
                />
              }
              ...
              >
                ...
                <Box>
                  ...
                  <TextField
                    ...
                    // }
                    value={selectedEvent.start.substr(0, 16)}
                  ...
                  <Box mt={2} display="flex" justifyContent="space-between">
                    ...
                    >
                      {selectedEvent && selectedEvent.id ? "Update" : "Add"}
                    </Button>
                    <Button
                      variant="outlined"
                      color="secondary"
